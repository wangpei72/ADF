...
class Local_Perturbation(object):
    ...
    def __call__(self, x):
        ...
        if np.zeros(self.input_shape).tolist() == ind_grad[0].tolist() and np.zeros(self.input_shape).tolist() == \
            ...
            grad_sum = 1.0 / (abs(ind_grad[0]) + abs(n_ind_grad[0])+0.000000001)
...
def dnn_fair_testing(dataset, sensitive_param, model_path, cluster_num, max_global, max_local, max_iter):
    ...
    if not os.path.exists('../results100/'):
        os.makedirs('../results100/')
    if not os.path.exists('../results100/' + dataset + '/'):
        os.makedirs('../results100/' + dataset + '/')
    if not os.path.exists('../results100/'+ dataset + '/'+ str(sensitive_param) + '/'):
        os.makedirs('../results100/' + dataset + '/'+ str(sensitive_param) + '/')

    # storing the fairness testing result
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/suc_idx.npy', np.array(suc_idx))
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/global_samples.npy', np.array(global_disc_inputs_list))
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/local_samples.npy', np.array(local_disc_inputs_list))
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/disc_value.npy', np.array(value_list))
