...
def aequitas(dataset, sensitive_param, model_path, max_global, max_local, step_size):
    ...
    if not os.path.exists('../results100/'):
        os.makedirs('../results1000/')
    if not os.path.exists('../results100/' + dataset + '/'):
        os.makedirs('../results1000/' + dataset + '/')
    if not os.path.exists('../results100/'+ dataset + '/'+ str(sensitive_param) + '/'):
        os.makedirs('../results100/' + dataset + '/'+ str(sensitive_param) + '/')

    # storing the fairness testing result
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/global_samples_aequitas.npy', np.array(global_disc_inputs_list))
    np.save('../results100/'+dataset+'/'+ str(sensitive_param) + '/disc_value_aequitas.npy', np.array(value_list))
    np.save('../results100/' + dataset + '/' + str(sensitive_param) + '/local_samples_aequitas.npy', np.array(local_disc_inputs_list))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('model_path', '../model/census/test.model', 'the path for testing model')
